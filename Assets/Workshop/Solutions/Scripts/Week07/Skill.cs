...
public class Skill
{
    ...
    public void Unlock()
    {
        if (!isAvailable)
        {
            ...
            throw new System.Exception("skill is not avaiblable to unlock");
        }
    ...
    public void PrintSkillTree()
    {
        ...
        // and call PrintSkillTree() on all nextSkill
        Debug.Log($"Skill: {name} availble: {isAvailable} unlock: {isUnlocked}");
        for (int i = 0; i < nextSkills.Count; i++)
        {
    ...
    public void PrintSkillTreeHierarchy(string indent)
    {
        ...
        Debug.Log($"{indent} -Skill: {name} available: {isAvailable} unlock: {isUnlocked}");
